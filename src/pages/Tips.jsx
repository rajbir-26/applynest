    ...
    <div className="min-h-screen bg-paper text-neutral-dark flex flex-col justify-between antialiased relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col min-h-screen justify-between">
        <div>
          ...
          <main className="max-w-6xl mx-auto px-6 py-12 animate-fade-up">
            
            {/* Header Banner */}
            <div className="text-center max-w-3xl mx-auto mb-12">
              <span className="text-xs font-bold text-[#944E2D] bg-[#FFF3EC] border border-[#FCDDCF] px-3 py-1 rounded-full uppercase tracking-wider inline-block hover:scale-105 transition-transform">
                Career Playbook
              </span>
            ...
            {/* Loading Indicator */}
            {loading ? (
              <div className="py-20 text-center text-gray-500 font-medium text-sm animate-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {categories.map((item) => (
                  <Link
                    key={item.id}
                    to={`/tips/${item.id}`}
                    className="bg-white/80 backdrop-blur-sm border border-[#EAD9C9] hover:border-[#944E2D] rounded-3xl p-6 transition-all duration-300 shadow-sm hover:shadow-md hover:-translate-y-1 flex flex-col justify-between group cursor-pointer"
                  >
                    <div>
                      {/* Top Badge & Icon */}
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-3xl p-2.5 bg-[#F9F6F0] rounded-2xl border border-[#EAD9C9] group-hover:scale-105 transition-transform">
                          {item.icon}
                        </span>

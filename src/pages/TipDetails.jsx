...
export default function TipDetails() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-paper text-neutral-dark flex flex-col justify-between relative overflow-hidden">
        <BackgroundDecor />
          ...
          <main className="max-w-4xl mx-auto px-6 py-10 animate-fade-up">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {tip.youtubeVideos.map((video, idx) => (
                    <div key={idx} className="bg-white/80 backdrop-blur-sm border border-[#EAD9C9] rounded-2xl p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
              ...
              <div className="mb-8">
                ...
                <div className="space-y-4">
                  {tip.prompts.map((p, idx) => (
                    <div key={idx} className="bg-white/80 backdrop-blur-sm border border-[#EAD9C9] rounded-2xl p-5 shadow-sm transition-all duration-300 hover:shadow-md">
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="text-xs font-bold text-gray-900">{p.title}</h3>
                        <button
                          onClick={() => handleCopyPrompt(p.promptText, idx)}
                          className="inline-flex items-center gap-1.5 text-[11px] font-bold text-[#944E2D] bg-[#FFF3EC] hover:bg-[#FCDDCF] border border-[#FCDDCF] px-3 py-1.5 rounded-xl transition-all cursor-pointer active:scale-95 shadow-xs"
                        >
                          {copiedIndex === idx ? (
                            <>
            ...
            {tip.resources && tip.resources.length > 0 && (
              <div className="mb-8">
                ...
                <div className="bg-white/80 backdrop-blur-sm border border-[#EAD9C9] rounded-2xl p-4 shadow-sm divide-y divide-[#F2EDE4]">
                  {tip.resources.map((res, idx) => (
                    <div key={idx} className="py-3 first:pt-0 last:pb-0 flex items-center justify-between">
                      ...
                      <a
                        ...
                        className="inline-flex items-center gap-1 text-xs font-bold text-[#944E2D] hover:underline active:scale-95 transition-transform"
                      >
            ...
            {(!tip.youtubeVideos?.length && !tip.prompts?.length && !tip.resources?.length) && (
              <div className="bg-white/80 backdrop-blur-sm border border-[#EAD9C9] rounded-3xl p-8 text-center shadow-sm">
